...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        List<User> foundUsers = jdbcTemplate.query("SELECT * FROM users ORDER BY name, email", ROW_MAPPER);
        Map<Integer, Set<Role>> usersRoles = getAllUsersRoles();
        foundUsers.forEach(user -> user.setRoles(usersRoles.get(user.getId())));
        return foundUsers;
    }
    ...
    private Map<Integer, Set<Role>> getAllUsersRoles() {
        ...
        jdbcTemplate.query("SELECT * FROM user_roles", rs -> {
            usersRoles.computeIfAbsent(rs.getInt("user_id"), id -> EnumSet.noneOf(Role.class))
