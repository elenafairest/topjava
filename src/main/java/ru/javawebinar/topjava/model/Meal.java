...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.GET_BY_ID, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.id=:id AND m.user.id=:userId"),
        ...
        @NamedQuery(name = Meal.FILTERED_BY_DATE, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user WHERE m.user.id=:userId AND m.dateTime >=:startDateTime AND m.dateTime <:endDateTime ORDER BY m.dateTime DESC"),
})
@Entity
